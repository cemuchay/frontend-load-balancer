...
} from "@pkg/load-balancer";
const API_BASE = import.meta.env.VITE_API_URL || "http://localhost:4000";

const SERVERS: ServerConfig[] = [
   { id: "server1", url: `${API_BASE}/server1` },
   { id: "server2", url: `${API_BASE}/server2` },
   { id: "server3", url: `${API_BASE}/server3` },
];
export function useLoadBalancer() {
   ...
   const sendRequest = useCallback(async () => {
      try {
         ...
         setLogs((prev) =>
            [{ ...result, id: crypto.randomUUID() }, ...prev].slice(0, 10)
         );
      } catch (err) {
